...
const EditTrail = (props) => {
    ...
    const deleteTrail = ()=>{
        axios.delete(`http://localhost:8000/api/trails/${id}`)
            .then((res)=>{
                console.log(res);
                console.log(res.data);
    ...
    //get one trail's data to autofill fields
    useEffect(()=>{
    ...
    const submitHandler = (e)=>{
        e.preventDefault();
        axios.put(`http://localhost:8000/api/trails/${id}`,
        {
            title,
            location,
            difficulty,
            ...
            <div>
                <form onSubmit={submitHandler} className="d-flex justify-content-end text-info">
                    <div className="form-group col-5 my-5 me-5 rounded bg-opacity-75 bg-secondary p-3">
                        <h2 className="text-light">Edit {title}</h2>
                        ...
                        <br></br>
                        <button className="btn btn-success mx-4 mt-4">Update Hike</button>
                        <button className="btn btn-danger mx-4 mt-4" onClick={()=>deleteTrail(id)} >Delete Hike</button>
                    </div>
                </form>
            </div>
        </div>
